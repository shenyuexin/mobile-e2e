...
import {
	...
	resolveInterruptionWithMaestro,
	resumeInterruptedActionWithMaestro,
} from "./interruption-tools.js";
import { probeNetworkReadiness } from "./network-probe.js";
import { getSharedPageContextDetectorService } from "./page-context-service.js";
import {
...
export interface OcrFallbackTestHooks {
	...
	) => Promise<ToolResult<ScreenshotData>>;
...
export interface ActionOrchestratorDeps {
	...
	invalidatePageContextCache?: (sessionId: string) => void;
}
...
export function setOcrFallbackTestHooksForTesting(
	hooks: OcrFallbackTestHooks | undefined,
): void {
...
export function setInterruptionGuardTestHooksForTesting(
	hooks: InterruptionGuardTestHooks | undefined,
): void {
...
function isInterruptionGuardPassed(
	status: ResolveInterruptionData["status"] | undefined,
): boolean {
	return status === "resolved" || status === "not_needed";
}

function requiredManualHandoff(
	state?: StateSummary,
	...
	return uniqueNonEmpty(
		[
			...
			params.recommendation.summary
				? `manual_handoff_summary:${params.recommendation.summary}`
				: undefined,
			params.stateSummary?.protectedPage?.suspected
				? "protected_page_suspected:true"
				: undefined,
			params.stateSummary?.protectedPage?.observability
				? `protected_page_observability:${params.stateSummary.protectedPage.observability}`
				: undefined,
		],
		6,
	);
...
function buildManualHandoffNextSuggestions(
	recommendation?: ManualHandoffRecommendation,
): string[] {
	if (!recommendation) {
		return [];
...
export async function performActionWithEvidenceWithMaestro(
	...
	if (!platform) {
		return {
			...
			data: {
				...
				evidenceDelta: {
					uiDiffSummary:
						"Action was not executed because platform could not be resolved.",
				},
				lowLevelStatus: "failed",
				lowLevelReasonCode: REASON_CODES.configurationError,
			},
			nextSuggestions: [
				"Provide platform explicitly or start a session before calling perform_action_with_evidence.",
			],
		};
	}
	...
	if (!executeIntent) {
		return {
			...
			nextSuggestions: [
				"Action executor is unavailable in current runtime context.",
			],
		};
	}
	...
	const getScreenSummary =
		ocrFallbackTestHooks?.getScreenSummary ?? getScreenSummaryWithMaestro;
	const resolveInterruptionExecutor =
		interruptionGuardTestHooks?.resolveInterruption ??
		resolveInterruptionWithMaestro;
	const resumeInterruptionExecutor =
		interruptionGuardTestHooks?.resumeInterruptedAction ??
		((resumeInput: ResumeInterruptedActionInput) =>
			resumeInterruptedActionWithMaestro(resumeInput, {
				executeIntentWithMaestro: executeIntent,
			}));
	...
	const preActionInterruption = await resolveInterruptionExecutor({
		...
		actionId,
		checkpoint: buildInterruptionCheckpoint(
			input.sessionId,
			platform,
			actionId,
			input.action,
	...
	if (!isInterruptionGuardPassed(preActionInterruption.data.status)) {
		const guardReasonCode =
			preActionInterruption.reasonCode === REASON_CODES.ok
		...
		return {
			...
			nextSuggestions:
				preActionInterruption.nextSuggestions.length > 0
	...
	const preActionPageContext = preStateResult.data.pageContext;
	const preActionInterruptionHint = preActionPageContext
		? classifyInterruptionFromPageContext(preActionPageContext)
		: undefined;
	...
	const lowLevelResult = manualHandoffPreblocked
		? {
				...
				artifacts: [] as string[],
				data: {},
				nextSuggestions: buildManualHandoffNextSuggestions(
					preActionManualHandoff,
				),
			}
		: await executeIntent(
				{
	...
	const ocrFallbackResult =
		!manualHandoffPreblocked &&
		canAttemptOcrFallback(input.action, lowLevelResult)
			? await executeOcrFallback({
					...
					deps: {
						...
						takeScreenshot:
							ocrFallbackTestHooks?.takeScreenshot ?? takeScreenshotWithRuntime,
						tap: ocrFallbackTestHooks?.tap ?? tapWithMaestroTool,
						getScreenSummary:
							ocrFallbackTestHooks?.getScreenSummary ??
							getScreenSummaryWithMaestro,
	...
	let actionStatus = ocrFallbackResult?.attempted
		? ocrFallbackResult.status
		: lowLevelResult.status;
	let actionReasonCode = ocrFallbackResult?.attempted
		? ocrFallbackResult.reasonCode
		: lowLevelResult.reasonCode;
	const fallbackUsed = Boolean(ocrFallbackResult?.used);
	const postStateResult = manualHandoffPreblocked
		? preStateResult
		: (ocrFallbackResult?.postStateResult ??
			(await getScreenSummary({
				sessionId: input.sessionId,
				platform,
				runnerProfile,
				harnessConfigPath: input.harnessConfigPath,
	...
	let manualHandoffContext:
		...
		? preActionManualHandoff
			? {
					phase: "pre" as const,
					recommendation: preActionManualHandoff,
					stateSummary: preStateSummary,
				}
			: undefined
		: undefined;
	if (!manualHandoffContext) {
		...
		if (postActionManualHandoff) {
			manualHandoffContext = {
				phase: "post" as const,
				recommendation: postActionManualHandoff,
	...
	const maxRetryAttempts =
		stepState === "recoverable_waiting" || stepState === "partial_progress"
			? 3
			: stepState === "degraded_but_continue_safe"
				? 2
				: 1;
	let retryAttemptIndex = 1;
	let retryStopReason: string | undefined = manualHandoffContext
	...
	while (
		...
		if (retryManualHandoff) {
			manualHandoffContext = {
				phase: "post",
				recommendation: retryManualHandoff,
				stateSummary: postStateSummary,
			};
	...
	if (
		!stateChanged &&
		retryAttemptIndex >= maxRetryAttempts &&
		(stepState === "recoverable_waiting" ||
			stepState === "partial_progress" ||
			stepState === "degraded_but_continue_safe")
	) {
		retryStopReason = "retry_budget_exhausted_without_state_change";
		actionStatus = "failed";
		actionReasonCode =
			postStateSummary.readiness === "waiting_network"
	...
	if (
		...
		if (hasStateChanged(postStateSummary, refreshedPostStateSummary)) {
			...
			if (refreshedManualHandoff) {
				manualHandoffContext = {
					phase: "post",
					recommendation: refreshedManualHandoff,
					stateSummary: postStateSummary,
				};
				actionStatus = "partial";
				actionReasonCode = REASON_CODES.manualHandoffRequired;
	...
	const outcome: ActionOutcomeSummary = {
		...
		fallbackUsed,
		retryCount:
			(ocrFallbackResult?.retryCount ?? 0) + Math.max(0, retryAttemptIndex - 1),
		stepState,
		progressMarker: classifyActionProgressMarker({
			finalStatus: actionStatus,
			stateChanged,
			postconditionStatus:
				postActionVerificationTrace.postconditionStatus ??
				classifyPostconditionStatus({
					actionType: input.action.actionType,
					finalStatus: actionStatus,
					stateChanged,
		...
		failureCategory,
		confidence:
			ocrFallbackResult?.ocrEvidence?.ocrConfidence ??
			buildActionOutcomeConfidence(actionStatus, stateChanged),
	...
	const actionEvent: SessionTimelineEvent = {
		...
		evidenceCompleteness: {
			...
			missingEvidence:
				artifacts.length > 0 ? [] : ["pre/post evidence was not captured"],
		},
	};
	...
	if (sessionRecord && retryTimelineMarkers.length > 0) {
		await persistSessionState(
			...
			postStateSummary,
			{
				...
				summary: retryDecisionTrace.retryAllowed
					? "retry_allowed"
					: "retry_stopped",
				artifactRefs: artifacts,
				stateSummary: postStateSummary,
		...
		await recordBaselineEntry(repoRoot, {
			...
			replayValue:
				outcome.outcome === "success"
					? outcome.progressMarker === "full" ||
						outcome.postconditionStatus === "met"
						? "high"
						: "medium"
					: "low",
			...
			evidenceFingerprint:
				uniqueNonEmpty(
					[
						evidenceDelta.uiDiffSummary
							? `ui:${evidenceDelta.uiDiffSummary}`
							: undefined,
						evidenceDelta.networkDeltaSummary
							? `network:${evidenceDelta.networkDeltaSummary}`
							: undefined,
						evidenceDelta.runtimeDeltaSummary
							? `runtime:${evidenceDelta.runtimeDeltaSummary}`
							: undefined,
						evidenceDelta.logDeltaSummary
							? `log:${evidenceDelta.logDeltaSummary}`
							: undefined,
					],
					4,
				).join("|") || undefined,
	...
	if (
		...
		preActionInterruptionHint.type !== "unknown"
	) {
		actionabilityReview.unshift(
			`pre_action_page_context:${preActionPageContext?.type}`,
			`pre_action_interruption_hint:${preActionInterruptionHint.type}`,
		);
	}
	if (postActionRefreshAttempted) {
		const refreshSignal =
			refreshedPostStateSummary &&
			hasStateChanged(
				postStateResult.data.screenSummary,
				refreshedPostStateSummary,
			)
		...
		if (refreshSignal === "post_action_refresh_no_additional_change") {
			...
			actionabilityReview.unshift(
				actionabilityReview.some((item) =>
					item.startsWith("stale_state_candidate:"),
				)
	...
	if (manualHandoffContext) {
		actionabilityReview.unshift(
			...buildManualHandoffReview(manualHandoffContext).reverse(),
		);
	}
	...
	let allArtifacts = persistedAction.relativePath
		? [persistedAction.relativePath, ...artifacts]
		: artifacts;
	...
	allArtifacts = Array.from(
		new Set([...allArtifacts, ...postActionInterruption.artifacts]),
	);

	if (!isInterruptionGuardPassed(postActionInterruption.data.status)) {
		finalToolStatus = "failed";
		finalToolReasonCode =
			postActionInterruption.reasonCode === REASON_CODES.ok
				? REASON_CODES.interruptionResolutionFailed
		...
		finalActionabilityReview.unshift(
			`post_interruption_status:${postActionInterruption.data.status}`,
		);
		...
		if (!resumed.data.resumed) {
			finalToolStatus = "failed";
			finalToolReasonCode =
				resumed.reasonCode === REASON_CODES.ok
	...
	if (
		finalToolStatus !== actionStatus ||
		finalToolReasonCode !== actionReasonCode
	) {
		...
		if (persistedAfterInterruption.relativePath) {
			allArtifacts = Array.from(
				new Set([persistedAfterInterruption.relativePath, ...allArtifacts]),
			);
		}
	}
	...
	if (stateChanged) {
		(
			deps.invalidatePageContextCache ??
			((sessionId: string) =>
				getSharedPageContextDetectorService().clearSession(sessionId))
		)(input.sessionId);
	}
	...
	if (shouldProbeNetwork) {
		...
		const probeDeviceId =
			input.deviceId ?? probeSessionRecord?.session.deviceId;
	...
	const networkNextSuggestions =
		networkRecoveryStrategyResult &&
		networkRecoveryStrategyResult.strategy !== "none"
			? [
	...
	return {
		...
		data: {
			...
			timelineDecisionMarkers: uniqueNonEmpty(
				[
					...
					postActionInterruption.data.status !== "not_needed"
						? `post_interruption:${postActionInterruption.data.status}`
						: undefined,
				],
				8,
			),
		...
		nextSuggestions: uniqueNonEmpty(
			[
				...
				...buildManualHandoffNextSuggestions(
					manualHandoffContext?.recommendation,
				),
				...buildRetryRecommendations({
