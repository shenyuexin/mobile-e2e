...
export function transformWdaElement(
	wdaElement: WdaElement,
): TransformedElement {
	return {
		...
		children: (wdaElement.children ?? []).map((child) =>
			transformWdaElement(child),
		),
	};
}
...
export class WdaRealDeviceBackend implements IosExecutionBackend {
	...
	async probePreflightReadiness(
		...
		try {
			...
			if (!response.ok) {
				return {
					available: false,
					error: `WDA /status returned ${response.status}`,
				};
			}
			...
			return {
				available: true,
				version: sessionId ? `session:${sessionId.slice(0, 8)}` : "ready",
			};
		} catch (error) {
			return {
				available: false,
				error: `WDA not reachable at ${this.getBaseUrl()}: ${error instanceof Error ? error.message : String(error)}`,
			};
		}
	}

	async probeAvailability(_repoRoot: string): Promise<BackendProbeResult> {
		return this.probePreflightReadiness("probe");
	}
	...
	buildTypeTextCommand(deviceId: string, text: string): string[] {
		return [
			...
			"POST",
			"/wda/keys",
			JSON.stringify({ value: text.split("") }),
		];
	}
	...
	async executeWdaRequest(
		_deviceId: string,
		method: string,
		path: string,
		body?: unknown,
	): Promise<WdaExecutionResult> {
		...
		try {
			...
			if (!response.ok) {
				return {
					success: false,
					error: `WDA ${method} ${path} returned ${response.status}`,
				};
			}
			const data = (await response.json()) as WdaResponse;
			...
			return {
				success: false,
				error: `WDA request failed: ${error instanceof Error ? error.message : String(error)}`,
			};
		}
	}
	...
	buildFailureSuggestion(action: string, _deviceId: string): string {
		const suggestions: Record<string, string> = {
			...
			typeText:
				"Check WDA is running and a text field is focused on the device.",
			swipe: "Check WDA is running and coordinates are valid.",
			hierarchy:
				"Check WDA is running. Try 'curl http://localhost:8100/source' manually.",
			screenshot: "Check WDA is running.",
		};
		return (
			suggestions[action] ??
			`Check WDA connectivity. Run 'curl http://localhost:8100/status' to verify.`
		);
	}
}
