...
function buildIosToolCapabilities(): ToolCapability[] {
  ...
  buildToolCapability("tap", PARTIAL, `Direct iOS coordinate tap uses idb on simulator targets and a generated Maestro action flow on physical devices; physical-device execution remains signing-dependent and proof-gated. ${IOS_PROOF_GATE_NOTE}`, true, iosProofGate),
  buildToolCapability("tap_element", PARTIAL, `iOS element tap resolves targets through idb-backed hierarchy capture, then executes direct tap (idb on simulator / generated Maestro action flow on physical devices). Physical-device execution remains proof-gated. ${IOS_PROOF_GATE_NOTE}`, true, iosProofGate),
    buildToolCapability("terminate_app", FULL, "iOS simulator app termination is supported."),
  buildToolCapability("type_text", PARTIAL, `Direct iOS text input uses idb on simulator targets and a generated Maestro action flow on physical devices; physical-device execution remains signing-dependent and proof-gated. ${IOS_PROOF_GATE_NOTE}`, true, iosProofGate),
  buildToolCapability("type_into_element", PARTIAL, `iOS element text input resolves targets through idb-backed hierarchy capture, then executes direct text input (idb on simulator / generated Maestro action flow on physical devices). Physical-device execution remains proof-gated. ${IOS_PROOF_GATE_NOTE}`, true, iosProofGate),
