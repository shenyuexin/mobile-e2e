...
function buildIosToolCapabilities(): ToolCapability[] {
  ...
  return [
    ...
    buildToolCapability("inspect_ui", PARTIAL, `iOS hierarchy capture uses axe describe-ui (simulators) or Maestro (physical devices). Query and action parity is full for simulators with axe, partial for physical devices. ${IOS_PROOF_GATE_NOTE}`, true, iosProofGate),
    buildToolCapability("query_ui", FULL, "iOS query_ui filters captured hierarchy nodes through axe-backed hierarchy (simulators) or Maestro snapshot (physical devices)."),
    buildToolCapability("resolve_ui_target", FULL, "iOS target resolution uses axe-backed hierarchy for simulators and Maestro-backed for physical devices."),
    buildToolCapability("scroll_and_resolve_ui_target", FULL, "iOS scroll-assisted target resolution uses axe swipe for simulators and Maestro swipe flow for physical devices."),
  ...
  buildToolCapability("tap", PARTIAL, `Direct iOS coordinate tap uses axe (simulators) or Maestro flow YAML (physical devices). Physical-device execution remains signing-dependent and proof-gated. ${IOS_PROOF_GATE_NOTE}`, true, iosProofGate),
  buildToolCapability("tap_element", PARTIAL, `iOS element tap resolves targets through axe-backed hierarchy (simulators) or Maestro flow (physical devices), then executes tap. Physical-device execution remains proof-gated. ${IOS_PROOF_GATE_NOTE}`, true, iosProofGate),
    buildToolCapability("terminate_app", FULL, "iOS simulator app termination is supported."),
  buildToolCapability("type_text", PARTIAL, `Direct iOS text input uses axe (simulators) or Maestro flow YAML (physical devices). Physical-device execution remains signing-dependent and proof-gated. ${IOS_PROOF_GATE_NOTE}`, true, iosProofGate),
  buildToolCapability("type_into_element", PARTIAL, `iOS element text input resolves targets through axe-backed hierarchy (simulators) or Maestro flow (physical devices), then executes text input. Physical-device execution remains proof-gated. ${IOS_PROOF_GATE_NOTE}`, true, iosProofGate),
    buildToolCapability("wait_for_ui", FULL, "iOS wait_for_ui polls axe hierarchy capture for simulators; physical device wait uses Maestro snapshot."),
