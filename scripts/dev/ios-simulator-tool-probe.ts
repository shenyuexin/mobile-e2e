...
function toMarkdown(params: {
  ...
  return [
    "# iOS Simulator Tool Probe Report", "",
...
export async function runIosSimulatorToolProbe(): Promise<void> {
  ...
  const latestJsonPath = join(reportsDir, "ios-simulator-tool-probe.json");
  const latestMdPath = join(reportsDir, "ios-simulator-tool-probe.md");
