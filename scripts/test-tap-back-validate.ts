/**
 * Test: tap General, then navigate back, verify page validation
 */
...
async function main(): Promise<void> {
  ...
  const sessionId = "test-tap-back-" + Date.now();
  ...
  console.log("3. Tapping General...");
  const selector = generalElement.selector;
  const tapArgs: any = {};
  if (selector.accessibilityId) tapArgs.accessibilityId = selector.accessibilityId;
  ...
  console.log("4. Capturing General page snapshot...");
  ...
  console.log("5. Navigate back to Settings...");
  ...
  console.log("6. Checking if we're on expected page...");
  const onExpectedPage = await backtracker.isOnExpectedPage(homeSnapshot.screenId);
  console.log(`   On expected page (screenId=${homeSnapshot.screenId}): ${onExpectedPage ? "✅ YES" : "❌ NO"}`);
  ...
  if (!onExpectedPage) {
    ...
    console.log(`   Match: ${afterBackSnapshot.screenId === homeSnapshot.screenId ? "✅ YES" : "❌ NO"}`);
  }
