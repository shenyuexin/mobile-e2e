...
- [x] **Phase 7: iOS Parity Hardening** - Stage the runtime, evidence, and acceptance work needed to move iOS toward the Android execution baseline without overclaiming support.
- [x] **Phase 8: iOS Real-Device Record/Replay Lane** - Build and prove a physical-device iOS record/replay lane so support promotion can rely on simulator + real-device evidence rather than simulator-only confidence.
- [x] **Phase 9: iOS Real-Device Capability Promotion Hardening** - Harden direct physical-device action paths and keep capability/descriptors/docs aligned with proof-gated runtime truth.
- [ ] **Phase 10: Android Real-Device Replay Dependency Reduction** - Reduce Android real-device replay reliance on Maestro helper-app installs by introducing a self-owned primary replay lane with explicit fallback boundaries.
- [x] **Phase 11: Diagnostics and Crash Evidence Deepening** - Close the gap in log/crash/diagnostics evidence collection: pull real ANR trace content, support iOS physical-device crash logs, and upgrade AI-consumable crash attribution.
...
| 8. iOS Real-Device Record/Replay Lane | 1/1 | Completed | 2026-04-03 |
| 9. iOS Real-Device Capability Promotion Hardening | 2/2 | Completed | 2026-04-06 (09-02) |
...
### Phase 8: iOS Real-Device Record/Replay Lane

**Goal**: Close the remaining iOS proof gap by implementing and validating a physical-device record/replay lane that is distinct from simulator-only confidence.
**Depends on:** Phase 7
**Success Criteria** (what must be TRUE):
  1. iOS recording runtime supports a bounded physical-device capture path with explicit failure and fallback semantics.
  2. iOS replay output from physical-device recordings is evidence-backed and regression-tested for selector confidence and degradation behavior.
  3. Acceptance artifacts keep simulator and real-device iOS lanes explicit, and support-boundary updates remain proof-gated.
**Plans**: 1 plan

Plans:
- [x] 08-01: Establish iOS physical-device record/replay implementation and proof plan

### Phase 9: iOS real-device capability promotion hardening

**Goal:** Harden iOS direct physical-device action execution and synchronize runtime truth across capability model, MCP descriptors, tests, and public guidance without over-promoting support boundaries.
**Requirements**: [promotion-gate hardening slice]
**Depends on:** Phase 8
**Plans:** 2 plans

Plans:
- [x] 09-01: Define and harden iOS real-device promotion gates from partial to evidence-ready
- [x] 09-02: Build self-owned iOS physical-device tap/type executor with deterministic crash attribution

### Phase 10: Android real-device replay dependency reduction

**Goal:** Minimize Android real-device replay friction by making helper-app installation an explicit fallback requirement rather than the primary replay gate.
**Requirements**: [android replay dependency reduction slice]
**Depends on:** Phase 9
**Plans:** 1 plan

Plans:
- [ ] 10-01: Reduce Android real-device replay dependency on Maestro helper apps

### Phase 11: Diagnostics and Crash Evidence Deepening

**Goal:** Close the gaps in log/crash/diagnostics evidence collection so AI agents receive complete, AI-consumable crash and ANR evidence from both Android and iOS without manual file-pull steps.
**Depends on:** Phase 7 (iOS parity baseline), Phase 9 (iOS real-device action path)
**Plans:** 4 plans

Plans:
- [x] 11-01: Pull Android ANR trace content instead of filename-only listing
- [x] 11-02: Add iOS physical-device crash log collection via `idevicesyslog` / `idevicecrashreport`
- [x] 11-03: Upgrade crash signal summary to structured AI-consumable crash attribution
- [x] 11-04: Add bounded `adb pull` trace fallback with timeout and size guardrails

### Phase 12: Action-Integrated Crash Attribution and Log Filtering

**Goal:** Auto-attach crash attribution to action outcomes, add log severity filtering, fix iOS syslog semantics, parallelize size checks, extract shared utilities, fix silent error swallowing, and reduce type casting.
**Depends on:** Phase 11 (crash attribution foundation)
**Plans:** 10 plans (12-05 split into 12-05a/12-05b, 12-06 split into 12-06a/12-06b)

Plans:
- [x] 12-01: Auto-attach crash attribution to performActionWithEvidence post-state
- [x] 12-02: Add minLogLevel and query filtering to get_logs
- [x] 12-03: Fix iOS syslog Tier 3 semantics and remove misleading fallback
- [x] 12-04: Parallelize size checks in boundedRemoteFileReadBatch
- [ ] 12-05: Extract shared utilities (isRecord, writeJsonFile, readNonEmptyString)
- [ ] 12-06: Fix silent error swallowing (empty catch in recording-store, tier fallback logging)
- [x] 12-07: Move inline types (ListDevicesData, EndSessionData) from server.ts to contracts
- [ ] 12-08: Reduce any/unknown casting in MCP server tool handler wrapping

### Phase 13: iOS Native Backend Router

**Goal:** Replace iOS idb dependency with xcrun simctl/devicectl native backend router, introduce IosExecutionBackend abstraction, and establish deterministic backend selection (env var > auto-detect > fallback chain).
**Requirements**: [TBD]
**Depends on:** Phase 12
**Plans:** 0 plans
...
### Phase 18: Workflow Optimization P0

**Goal:** Close the 4 highest-impact developer workflow gaps identified from workflow analysis: network-aware orchestration, multi-step checkpoint chain, flow validation before export, and element screenshot/visual baseline.
**Depends on:** Phase 12 (crash attribution foundation)
**Plans:** 4 plans

Plans:
- [x] 18-01: Network-aware orchestration (probe + recovery strategy + action orchestrator integration)
